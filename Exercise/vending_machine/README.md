...
## 19.01.19 추상화, range, 순환문, 함수, 컴프리핸션

배운 내용을 바탕으로 새로운 기능을 추가한다.  
이전의 프로그램에 추가한다.
> 1. 물품의 개수를 변경할 수 있는 admin모드를 추가  
> 1.1 일반 사용자는 확인할 수 없게 admin과 같은 특별한 문자열을 입력하면 접근할 수 있게 한다.
> 1.2 물품의 개수는 일반 사용자가 볼 수 없다.
> 2. admin모드에 접속하면 다음 선택창을 출력한다.  
> 1) 물품목록과 개수 출력  
> 2) 물품개수 변경  
> 3) Admin모드에서 나가기
> 3. 물품목록과 개수 출력을 누르면 물품목록과 개수를 출력하고 다시 선택창을 출력한다.
> 4. 물품개수 변경을 클릭하면 물품목록과 개수를 다시 출력하고, 물품을 선택했을 때, 물품 개수를 변경할 수 있게 한다.
> 5. 물품 개수 변경에서 'exit'와 같은 값을 입력하면, 물품 개수 변경에서 나갈 수 있게 만든다.
> 6. Admin모드에서 나가기를 선택하면 맨 처음 자판기 화면으로 돌아간다.
> 7. 물품을 뽑았을 대, 무품 개수가 부족하면 "물품이 부족합니다."를 출력하고 처음으로 되돌아간다.